...
# %%
class TrainEgoVLADeepSpeedWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        # Initialize accelerator with DeepSpeed ZeRO stage 2
        accelerator = Accelerator(
            log_with='wandb',
            mixed_precision='bf16',
            device_placement=True,
            # DeepSpeed configuration
            kwargs_handlers=[self._get_deepspeed_config()]
        )
        ...
        if accelerator.is_main_process:
            ...
            if torch.cuda.is_available():
                print(f"CUDA Device: {torch.cuda.get_device_name(accelerator.local_process_index)}")
                print(f"CUDA Capability: {torch.cuda.get_device_capability(accelerator.local_process_index)}")
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        # Gradient clipping (handled by DeepSpeed automatically, but we can add explicit clipping)
                        ...
                        # step optimizer (DeepSpeed handles gradient accumulation automatically)
                        ...
                        step_log = {
                            ...
                            'epoch': self.epoch,
                            'lr': lr_scheduler.get_last_lr()[0] if hasattr(lr_scheduler, 'get_last_lr') else 0.0
                        }
    ...
    def _get_deepspeed_config(self):
        """Get DeepSpeed configuration for ZeRO stage 2"""
        from accelerate.utils import DeepSpeedPlugin
        
        # Get gradient accumulation steps from config
        gradient_accumulation_steps = getattr(self.cfg.training, 'gradient_accumulate_every', 1)
...
def main(cfg):
    workspace = TrainEgoVLADeepSpeedWorkspace(cfg)
    workspace.run()
