...
#   bash scripts/run_eval.sh /path/to/ckpt.pt /path/to/.hydra/config.yaml
#
#   # RTC test: pin first 4 GT action steps as condition
#   bash scripts/run_eval.sh /path/to/ckpt.pt /path/to/.hydra/config.yaml inference_delay=4
#
#   # Custom output, fewer shards, smaller batch
#   bash scripts/run_eval.sh /path/to/ckpt.pt /path/to/.hydra/config.yaml \
#       num_shards=5 batch_size=8 output_dir=outputs/my_eval
#
