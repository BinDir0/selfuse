"""
Any4D depth inference for RoWaH SLAM (in-process; also runnable as CLI).
...
def _any4d_config_dict(any4d_root: str) -> dict:
    """Hydra-style config for RoWaH Any4D depth."""
...
def run_any4d_depth_batch(
    ...
    Expects RoWaH `extracted_images` next to the video stem folder (same as CLI).
    ...
    any4d_scripts_dir = os.path.join(any4d_root, "scripts")
    ...
    # Add Any4D paths so we can import its scripts and packages.
