...
@contextmanager
def eval_with_averaged_model(model, averaged_model):
...
def save_checkpoint_native(workspace, rank, path=None, tag='latest'):
    ...
    if rank == 0:
        ...
        print(
            ...
            f"(model + optimizer + lr_scheduler + training_state + model_averaging)"
        )
    dist.barrier()
    ...
    app_state = FSDPWorkspaceAppState(
        ...
        model_averaging=workspace.model_averaging,
    )
...
def evaluation(workspace, rank, device, dataloader, step_log):
    ...
    with (
        ...
        torch.no_grad(),
        eval_with_averaged_model(workspace.model, workspace.model_averaging),
    ):
