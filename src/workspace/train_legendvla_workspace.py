...
class TrainLegendVLAWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf):
        ...
        # Saving optimizer state costs real disk — skip it on fresh runs.
    ...
    def run(self):
        ...
        train_dataloader, val_dataloader, steps_per_epoch, topk_manager = self.setup_training(cfg, ctx)
        self.train_loop(cfg, ctx, profiler, train_dataloader, val_dataloader, steps_per_epoch, topk_manager)
        if ctx.rank == 0:
            wandb.finish()
    ...
    def setup_training(self, cfg, ctx):
        ...
        return train_dataloader, val_dataloader, steps_per_epoch, topk_manager
    ...
    def maybe_log_and_ckpt(
        ...
        # should_record uses the pre-increment update_step to match the old
    ...
    def train_loop(self, cfg, ctx, profiler, train_dataloader, val_dataloader, steps_per_epoch, topk_manager):
        rank = ctx.rank
        grad_accum_steps = int(cfg.training.get("gradient_accumulation_steps", 1))
