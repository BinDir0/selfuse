...
class TemporalCausalAttentionPass(nn.Module):
    ...
    Zero new learnable parameters. The spatial block's norm1 / qkv / out_proj
    are passed in at forward time so this module never registers them as its
    own children — that keeps each shared submodule registered exactly once
    (inside the spatial block) so FSDP doesn't see duplicate parameter
    references at the wrap boundary.
    ...
    def forward(
        ...
        # Add temporal PE BEFORE norm so that qkv_proj sees LN(z + e(t)),
        # matching MEM Appendix C: ẑ = z + e(t), then q/k/v = W · LN(ẑ).
        # The boundary condition e(0)=0 keeps single-frame inputs identical
        # to the pretrained spatial path.
        ...
        # Shared QKV projection (passed in to avoid registering as a child module)
        head_dim = qkv_proj.out_features // (3 * self.num_heads)
        qkv = qkv_proj(x)
        ...
        attn_out = out_proj(attn_out.reshape(M * N * T, -1))
...
class MEMVisionBlock(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 1. Temporal causal attention FIRST. Pass spatial block's norm1 / qkv /
        # proj at call time instead of stashing them on temporal_attn so they
        # stay registered exactly once (inside spatial_block) and FSDP doesn't
        # see duplicate references.
