...
class ModelInterfaceNode(Node):
    def __init__(self):
        ...
        # 音频服务配置
        self.declare_parameter('audio_service_host', 'localhost') # Docker中可能需要改为 172.17.0.1
        ...
        self.latest_obs = {'image': None, 'depth_image': None, 'left_wrist_pose': None, 'right_wrist_pose': None, 'left_keypoints': None, 'right_keypoints': None}
        ...
        # 5. 推理客户端
        try:
            self.policy_client = WebsocketClientPolicy(host=self.get_parameter('model_server_host').value, port=self.get_parameter('model_server_port').value)
        except Exception as e:
            self.get_logger().error(f"Server Connection Failed: {e}")

        # 6. 线程与定时器
        self.kbd_listener = keyboard.Listener(on_press=self.on_key_press); self.kbd_listener.start()
    ...
    def play_sound(self, audio_name):
        """异步发送音频播放请求到宿主机服务"""
        threading.Thread(target=self._request_audio, args=(audio_name,), daemon=True).start()

    def _request_audio(self, name):
        url = f"http://{self.audio_host}:{self.audio_port}/play/{name}"
        try:
            # 这里的 name 是不带后缀的，如 'start', 'pause'
            response = requests.post(url, timeout=0.5)
            if response.status_code != 200:
                self.get_logger().warn(f"Audio server returned error for {name}")
        except Exception as e:
            self.get_logger().debug(f"Could not reach audio server for {name}: {e}")
    ...
    def user_input_loop(self):
        while rclpy.ok():
            if self.state == SystemState.IDLE:
                ...
                mode_in = input("[Input] Mode (deploy/debug): ").strip().lower()
        ...
        if k == '1':
            with self.lock:
                if self.state == SystemState.READY:
                    self.play_sound("start") # 播放 start.mp3
            ...
            with self.lock:
                if self.state != SystemState.IDLE:
                    ...
                    self.play_sound("stop_and_reset") # 播放 stop_and_reset.mp3
    ...
    def inference_worker(self):
        while rclpy.ok():
            with self.lock:
                ...
                obs = self.latest_obs.copy(); instr = self.current_instruction
            ...
            try:
                ...
                payload = {
                    "image": obs['image'], "depth_image": obs['depth_image'], "camera_matrix": self.camera_matrix,
                    "instruction": instr,
                    "proprio": {
                ...
                with self.lock:
                    self.arm_queue.append(result); self.hand_queue.append(result)
    ...
    def arm_command_timer(self):
        ...
        try:
            pa = PoseArray(); pa.header.stamp, pa.header.frame_id = self.get_clock().now().to_msg(), "base_link"
    ...
    def hand_command_timer(self):
        ...
        def to_pa(kps, frame):
            pa = PoseArray(); pa.header.stamp, pa.header.frame_id = self.get_clock().now().to_msg(), frame
            for pt in np.array(kps).reshape(-1, 3):
                p = Pose(); p.position.x, p.position.y, p.position.z = map(float, pt)
                pa.poses.append(p)
            return pa
        try:
            if 'left' in data and 'keypoints' in data['left']: self.pub_action_hand_l.publish(to_pa(data['left']['keypoints'], "left_wrist_link"))
            if 'right' in data and 'keypoints' in data['right']: self.pub_action_hand_r.publish(to_pa(data['right']['keypoints'], "right_wrist_link"))
        except: pass
    ...
    def left_kp_cb(self, msg):
        with self.lock: self.latest_obs['left_keypoints'] = np.array([[p.position.x, p.position.y, p.position.z] for p in msg.poses])
    def right_kp_cb(self, msg):
        with self.lock: self.latest_obs['right_keypoints'] = np.array([[p.position.x, p.position.y, p.position.z] for p in msg.poses])
