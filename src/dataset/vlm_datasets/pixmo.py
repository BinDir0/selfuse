...
class PixmoDataset(BaseImageDataset):
    """
    Dataset class for the allenai/pixmo-cap dataset, loading from a local Parquet file.
    Inherits from BaseImageDataset.
    """
    def __init__(self, data_path=None, split='train', image_transform=None, text_transform=None):
        """
        Initializes the Pixmo dataset from a local Parquet file.
    ...
    def _load_data(self):
        """
        Loads Pixmo data from a local Parquet file.
        """
        print(f"Loading Pixmo data from local path: {self.data_path}")
...
if __name__ == '__main__':
    print("PixmoDataset class defined to load from a local Parquet file.")
    print("This class inherits from BaseDataset and is specialized for Pixmo.")
    ...
    local_parquet_path = "/share_data/datasets/VLM/pixmo-cap/data"

    print(f"\nAttempting to load 'train' split from: {local_parquet_path}...")
    
    # Pass the local path to the dataset class
    dataset = PixmoDataset(data_path=local_parquet_path)

    print(f"Successfully loaded Pixmo dataset metadata with {len(dataset)} samples.")
