...
class COCODataset(BaseImageDataset):
    """
    Dataset class for the lmms-lab/COCO-Caption2017 dataset, loading from a local Parquet file.
    Inherits from BaseImageDataset.
    """
    def __init__(self, data_path=None, split='train', image_transform=None, text_transform=None):
        """
        Initializes the COCO-Caption2017 dataset from a local Parquet file.
    ...
    def _load_data(self):
        """
        Loads COCO-Caption2017 data from a local Parquet file.
        """
        print(f"Loading COCO-Caption2017 data from local path: {self.data_path}")
    ...
    def __getitem__(self, idx):
        ...
        # sample['answer'] is a list → pick one randomly
        # if isinstance(sample['answer'], list):
        #     answer = random.choice(sample['answer'])
        # else:
        answer = sample['question_id']
...
if __name__ == '__main__':
    print("COCODataset class defined to load from a local Parquet file.")
    print("This class inherits from BaseDataset and is specialized for COCO-Caption2017.")
    ...
    local_parquet_path = "/share_data/datasets/VLM/COCO-Caption2017/data"

    print(f"\nAttempting to load 'train' split from: {local_parquet_path}...")
    
    # Pass the local path to the dataset class
    dataset = COCODataset(data_path=local_parquet_path)

    print(f"Successfully loaded COCO-Caption2017 dataset metadata with {len(dataset)} samples.")
