...
def _dual_view_raw_sample():
    return {
        ...
        "chest_extrinsic": np.eye(4, dtype=np.float32).reshape(-1),
        "chest_intrinsic": np.full(4, 2.0, dtype=np.float32),
        "instruction": ["pick up object"],
        "instruction_num": 1,
        "image": np.zeros((1, 4, 4, 3), dtype=np.uint8),
        "chest_image": np.full((1, 4, 4, 3), 99, dtype=np.uint8),
        "future_frames": np.full((2, 4, 4, 3), 7, dtype=np.uint8),
        "chest_future_frames": np.full((2, 4, 4, 3), 8, dtype=np.uint8),
    }
...
def test_sample_to_data_surfaces_chest_fields_when_chest_image_present():
    """Dual-view sample should produce chest_images/chest_intrinsic/chest_future_frames."""
    dataset = VLAWdsDataset(
        wds_datasets=[{"name": "demo", "shard_urls": "/tmp/unused/shard-*.tar"}],
        shape_meta=_shape_meta(),
        mode="val",
    ...
    assert "chest_images" in data
    assert data["chest_images"].shape == (1, 4, 4, 3)
    assert np.allclose(data["chest_intrinsic"], 2.0)
    assert "chest_future_frames" in data
    assert data["chest_future_frames"].shape == (3, 4, 4, 3)
    assert np.all(data["chest_future_frames"][:2] == 8)
    assert np.all(data["chest_future_frames"][2] == 0)
...
def test_view_dropout_keep_both_marks_both_views_active():
    dataset = VLAWdsDataset(
        ...
        mode="train",
        load_chest=True,
        view_dropout=ViewDropoutConfig(drop_head=0.0, drop_chest=0.0),
        target_image_size=(4, 4),
    )

    data = _sample_to_data_with_mocked_transforms(dataset, _dual_view_raw_sample())

    assert data["active_views"] == ["head", "chest"]
    assert data["view_mask"].tolist() == [True, True]
    assert "chest_images" in data
    assert "chest_future_frames" in data
    assert "future_chest_motion" in data


def test_view_dropout_drop_chest_keeps_only_head_active():
    dataset = VLAWdsDataset(
        wds_datasets=[{"name": "demo", "shard_urls": "/tmp/unused/shard-*.tar"}],
        shape_meta=_shape_meta(),
        mode="train",
        load_chest=True,
        view_dropout=ViewDropoutConfig(drop_head=0.0, drop_chest=1.0),
        target_image_size=(4, 4),
    )
...
def test_view_dropout_drop_head_keeps_only_chest_active():
    dataset = VLAWdsDataset(
        wds_datasets=[{"name": "demo", "shard_urls": "/tmp/unused/shard-*.tar"}],
        shape_meta=_shape_meta(),
        mode="train",
        load_chest=True,
        view_dropout=ViewDropoutConfig(drop_head=1.0, drop_chest=0.0),
        target_image_size=(4, 4),
    )

    data = _sample_to_data_with_mocked_transforms(dataset, _dual_view_raw_sample())

    assert data["active_views"] == ["chest"]
...
def test_sample_to_data_marks_head_only_when_chest_absent():
    """Head-only samples still carry view metadata and zero WM chest targets."""
    dataset = VLAWdsDataset(
        wds_datasets=[{"name": "demo", "shard_urls": "/tmp/unused/shard-*.tar"}],
        shape_meta=_shape_meta(),
        mode="val",
    ...
    assert "chest_images" not in data
    assert "chest_intrinsic" not in data
    assert data["active_views"] == ["head"]
    assert data["view_mask"].tolist() == [True, False]
    assert "chest_future_frames" in data
    assert np.all(data["chest_future_frames"] == 0)
    assert np.all(data["future_chest_motion"] == 0)
