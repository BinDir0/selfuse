...
def _default_arm_xml_path() -> str:
    ...
    return '/root/workspace/legendvla-inference/assets/PsiRobot_DC_02_OnlyArm/meshes/psi_robot_scene_transformed.xml'
...
class ArmFKNode(Node):
    def __init__(self):
        ...
        if self.arm_side not in ['left', 'right']:
            raise ValueError(f'不支持的 arm_side: {self.arm_side}')
        if not os.path.exists(self.arm_xml_path):
            raise FileNotFoundError(f'机械臂 FK XML 不存在: {self.arm_xml_path}')

        self.model = mujoco.MjModel.from_xml_path(self.arm_xml_path)
        ...
        self.get_logger().info(f'ArmFKNode ready for {self.arm_side} arm, xml={self.arm_xml_path}')

    def joint_state_callback(self, msg: JointState):
        if len(msg.position) != 7:
            self.get_logger().warn(f'期望 7 维关节角，收到 {len(msg.position)} 维，忽略。')
            return
