"""Offline dataset evaluation for LegendVLA flow matching.
...
def build_eval_inputs(batch, device, dtype):
    ...
    Mirrors LegendVLAInference.build_model_inputs (flow mode branch).
    # Source: src/policy/legendvla_inference_wrapper.py#L260-L288
    """
    inputs = {
...
def main(eval_cfg):
    ...
    if eval_cfg.flow_steps is not None:
        ...
        if model.diffloss is not None:
            model.diffloss.num_inference_steps = int(eval_cfg.flow_steps)
    ...
    # ---- 3. Load normalizer ----
    normalizer = load_normalizer(train_cfg.training.normalizer_path)
