#!/usr/bin/env python3
"""Find tar shards containing invalid .npy payloads."""
...
def _skip_payload(handle, nbytes: int, file_size: int | None) -> None:
    """Skip bytes without reading them into Python."""
    if nbytes <= 0:
        return
    current = handle.tell()
...
def check_one_sequential(
    tar_path: str,
    limit_npy: int | None = None,
    stop_after_first_bad: bool = True,
) -> tuple[str, list[tuple[str, str]], int]:
    """Forward tar scan with seek-based payload skips.

    This is exact for uncompressed .tar files. It checks every selected .npy
    payload's first six bytes unless --limit-npy-per-shard is used.
    """
    ...
    try:
        ...
        with open(tar_path, "rb") as handle:
            while True:
                ...
                if len(header) == 0:
                    break
                if len(header) < TAR_BLOCK:
                    return tar_path, [("__tar__", "truncated_header")], npy_checked
                ...
                if typeflag == "L":
                    ...
                    if len(payload) < size:
                        return tar_path, [("__tar__", "truncated_longname")], npy_checked
                ...
                if typeflag in ("0", "\0"):
                    ...
                    if effective_name.endswith(".npy"):
                        head = handle.read(min(len(NPY_MAGIC), size))
                        if len(head) < len(NPY_MAGIC):
                            bad.append((effective_name, f"short_read_{len(head)}"))
                        elif head != NPY_MAGIC:
                            bad.append((effective_name, f"bad_magic_{head.hex()}"))
                        npy_checked += 1
                        rest = size - len(head)
                        if rest:
                            _skip_payload(handle, rest, file_size)
                        _read_padding(handle, size, file_size)
                        if bad and stop_after_first_bad:
                            return tar_path, bad, npy_checked
                        if limit_npy is not None and npy_checked >= limit_npy:
                            break
                    else:
                        _skip_payload(handle, size, file_size)
                        _read_padding(handle, size, file_size)
        ...
        return tar_path, [("__tar__", str(error))], npy_checked
...
def check_one_tarfile(
    ...
    """tarfile fallback; slower on remote filesystems."""
    bad: list[tuple[str, str]] = []
    npy_checked = 0
    try:
            ...
            for member in tar_reader:
                ...
                try:
                    ...
                    if member_file is None:
                        ...
                        head = member_file.read(len(NPY_MAGIC))
                        if len(head) < len(NPY_MAGIC):
                            bad.append((member.name, f"short_read_{len(head)}"))
                        elif head != NPY_MAGIC:
                            bad.append((member.name, f"bad_magic_{head.hex()}"))
                except Exception as error:  # noqa: BLE001
...
def check_one_mmap(
    ...
    """Memory-mapped tar scan.

    This keeps the same exactness as the sequential parser for uncompressed
    .tar files, but avoids one read/seek syscall per tar member.
    """
    bad: list[tuple[str, str]] = []
    npy_checked = 0
    pending_long_name: str | None = None
    ...
    try:
        file_size = os.path.getsize(tar_path)
        if file_size == 0:
            return tar_path, [("__tar__", "empty_file")], npy_checked
        with open(tar_path, "rb") as handle:
            with mmap.mmap(handle.fileno(), 0, access=mmap.ACCESS_READ) as mm:
                pos = 0
                while pos < file_size:
                    if pos + TAR_BLOCK > file_size:
                        return tar_path, [("__tar__", "truncated_header")], npy_checked
                    header = mm[pos : pos + TAR_BLOCK]
                    pos += TAR_BLOCK
                    ...
                    next_pos = payload_end + _payload_padding(size)
                    if next_pos > file_size:
                        return tar_path, [("__tar__", "truncated_payload")], npy_checked

                    if typeflag == "L":
                        payload = mm[payload_start:payload_end]
                        pending_long_name = payload.split(b"\x00", 1)[0].decode("utf-8", errors="replace")
                        pos = next_pos
                        continue
                    ...
                    if typeflag in ("0", "\0"):
                        ...
                        pending_long_name = None
                        if effective_name.endswith(".npy"):
                            head_end = min(payload_start + len(NPY_MAGIC), payload_end)
                            head = mm[payload_start:head_end]
                            if len(head) < len(NPY_MAGIC):
                                bad.append((effective_name, f"short_read_{len(head)}"))
                            elif head != NPY_MAGIC:
                                bad.append((effective_name, f"bad_magic_{head.hex()}"))
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Scan shard-*.tar files and report shards with invalid .npy magic bytes."
    )
    ...
    parser.add_argument(
        ...
        default="thread",
        help="thread is usually best for tar scans on NFS/EFS",
    )
    parser.add_argument(
        "--engine",
        choices=("sequential", "mmap", "tarfile"),
        default="mmap",
        help="mmap is usually fastest; sequential is a low-memory fallback; tarfile is slow fallback",
    )
    parser.add_argument(
        "--limit-npy-per-shard",
        type=int,
        default=None,
        metavar="N",
        help="only verify first N .npy per shard; this is not a full audit",
    )
    parser.add_argument(
        "--full-detail",
        action="store_true",
        help="scan all .npy members in a bad shard and record up to --max-issues-per-shard details",
    )
    parser.add_argument(
    ...
    parser.add_argument("-o", "--out", type=Path, default=Path("bad_npy_shards.txt"))
...
def main() -> None:
    ...
    limit_text = f" limit_npy/shard={args.limit_npy_per_shard}" if args.limit_npy_per_shard else ""
    ...
    print(
        ...
        f"engine={args.engine} {detail_text}{limit_text}",
        flush=True,
    )
    ...
    limit_iter = itertools.repeat(args.limit_npy_per_shard)
    ...
    with executor_cls(max_workers=args.jobs) as executor:
        ...
        for path, issues, npy_checked in result_iter:
            done += 1
            npy_checked_total += int(npy_checked)
            if issues:
                bad_shard_count += 1
            ...
            if done % progress_every == 0 or done == len(shards):
                ...
                print(
                    ...
                    f"npy_checked={npy_checked_total} rate={rate:.2f} shard/s",
                    flush=True,
                )
    ...
    print(
        ...
        f"npy_checked={npy_checked_total} detail={args.out}",
        flush=True,
    )
