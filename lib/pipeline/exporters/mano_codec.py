...
DEFAULT_MANO_DIR = "/share_data/guantianrui/manopth/mano/models/"
MANO_PCA_DIMS = 45
MANO_BETA_DIMS = 10
...
def _resolve_mano_root(mano_dir: str | None = None) -> Path:
    base = Path(mano_dir or DEFAULT_MANO_DIR).expanduser()
    candidates = []
    if base.is_file():
    ...
    raise FileNotFoundError(
        f"Failed to resolve MANO model root from {base}. Expected MANO_RIGHT.pkl and MANO_LEFT.pkl."
    )
...
def _mano_pkl_path(side: str, mano_dir: str | None = None) -> str:
    ...
    return str(_resolve_mano_root(mano_dir) / file_name)


@lru_cache(maxsize=8)
def _load_pca_codec(side: str, mano_root: str, flat_hand_mean: bool, ncomps: int):
    with open(_mano_pkl_path(side, mano_root), "rb") as handle:
...
def hand_pose_axis_angle_to_pca(
    ...
    *,
    ...
    components, mean, projector = _load_pca_codec(side, str(_resolve_mano_root(mano_dir)), flat_hand_mean, ncomps)
    del components
...
def hand_pose_pca_to_axis_angle(
    ...
    *,
    ...
    components, mean, _ = _load_pca_codec(side, str(_resolve_mano_root(mano_dir)), flat_hand_mean, ncomps)
