...
class Any4DDepthEstimator:
    ...
    def prepare_holoassist_dataset(self):
        # 1. 扫描目录获取所有 Task Name
        video_root_base = "/share_data/guantianrui/datasets/HoloAssist/raw_data/new_video"
        print(f"Scanning HoloAssist directories in {video_root_base} ...")
        
        if not os.path.exists(video_root_base):
             raise FileNotFoundError(f"Root not found: {video_root_base}")
        
        # 获取第一级子目录名作为 task_name
        # 过滤掉非文件夹
        all_tasks = [
            d for d in os.listdir(video_root_base) 
            if os.path.isdir(os.path.join(video_root_base, d))
            and (d.startswith("R") or d.startswith("z"))
        ]
        all_tasks.sort() # 排序保证多卡一致性
        ...
        self.my_task_list = all_tasks[start_idx:end_idx]
    ...
    def holoassist_loop(self):
        ...
        if len(dataset) == 0:
            print("No HoloAssist tasks found.")
            return
        ...
        # Buffer 中我们存储：
        # (chunk_id, depth_array, c2w_pose_array)
        # 注意存 c2w 而不是 w2c，方便计算对齐
        ...
        for batch in tqdm(loader, desc="HoloAssist Progress"):
            ...
            # HoloAssist 我们保留整个 chunk (包括 overlap)，在最后拼接时再裁剪
            # 这样才能有重叠帧用于计算对齐
            ...
            for i in range(current_len):
                ...
                # 位姿 (Cam2World)
                cam_quats = pred_result[key_name]['cam_quats'][0].cpu() # (4,)
                cam_trans = pred_result[key_name]['cam_trans'][0].cpu() # (3,)
                
                # 转矩阵
                cam_rot = quaternion_to_rotation_matrix(cam_quats[None])[0]
                cam_pose = np.eye(4)
                cam_pose[:3, :3] = cam_rot.numpy()
                cam_pose[:3, 3] = cam_trans.numpy()
                
                chunk_c2w_poses.append(cam_pose)
            
            # 存入 Buffer
            buffer[name].append({
                "id": chunk_id,
                "depth": chunk_depths,   # list of (H, W)
                "pose": chunk_c2w_poses, # list of (4, 4) arrays
                "s_idx": s_idx,
                "e_idx": e_idx
            })
            ...
            # =================== 拼接与对齐逻辑 ===================
            if task["is_last_chunk"]:
                chunks = buffer.pop(name)
                # 按 id 排序
                chunks.sort(key=lambda x: x["id"])
                
                final_depth = []
                final_w2c_extrinsics = []
                
                # 累积变换矩阵 (Global Transform)
                # 初始为单位阵 (第一个 chunk 保持原样)
                T_accum = np.eye(4)
                
                for i, chunk in enumerate(chunks):
                    current_poses = np.array(chunk["pose"]) # (T, 4, 4) C2W
                    ...
                    if i > 0:
                        ...
                        # 上一个 chunk 的末尾 10 帧
                        # 当前 chunk 的开头 10 帧
                        # 注意：由于 dataset 切分逻辑，chunk[i] 的 start = chunk[i-1].end - overlap
                        # 所以 chunk[i] 的 0~10 帧 对应 chunk[i-1] 的 -10~end 帧
                        
                        # 严谨起见，我们应该用 s_idx 对齐
                        # overlap_len = prev_chunk["e_idx"] - chunk["s_idx"]
                        # 通常就是 self.overlap (10)，但最后一个 chunk 可能例外
                        
                        overlap_len = prev_chunk["e_idx"] - chunk["s_idx"]
                        
                        if overlap_len > 0:
                            # 提取用于计算对齐的 Pose
                            # Prev 尾部 overlap_len 帧 (已经对齐到全局了，因为 prev_chunk 已经处理过)
                            # 注意：我们需要用“已经对齐后的 Prev Pose”作为基准
                            # 但这里为了代码简单，我们先算出相对变换，再累乘
                            
                            # 这里有个 trick: 
                            # 我们已经把 T_accum 应用到了 prev_chunk 并存入了 final_w2c? 
                            # 不，我们还没存 pose。
                            # 让我们维护一个 aligned_poses_list 用于 prev
                            
                            # 取出原始预测的 pose (Local coordinate)
                            P_prev_local = np.array(prev_chunk["pose"])
                            P_curr_local = np.array(chunk["pose"])
                            
                            # 提取重叠部分 (Local)
                            P_ref_local_overlap = P_prev_local[-overlap_len:]
                            P_src_local_overlap = P_curr_local[:overlap_len]
                            
                            # 但我们要对齐的是： T_accum_prev @ P_src_local -> T_accum_prev @ P_ref_local
                            # 不对。
                            # 我们希望: T_new_accum @ P_src_local == T_prev_accum @ P_ref_local
                            # => T_relative @ P_src_local == P_ref_local
                            # => T_new_accum = T_prev_accum @ T_relative
                            
                            T_rel = compute_alignment_transform(
                                pose_ref=P_ref_local_overlap,
                                pose_src=P_src_local_overlap
                            )
                    ...
                    # 2. 应用对齐变换
                    # P_global = T_accum @ P_local
                    # current_poses 形状 (T, 4, 4)
                    # 矩阵乘法: (4,4) @ (T, 4, 4) -> (T, 4, 4)
                    # numpy 的 matmul 支持广播，但要注意维度顺序
                    
                    aligned_poses_c2w = np.matmul(T_accum[None, ...], current_poses)
                    
                    # 3. 裁剪并保存
                    # 策略：除了第一个 chunk，后续 chunk 都要扔掉开头重叠的部分 (overlap)
                    # 这样拼接起来是无缝的
                    if i == 0:
                        start_save = 0
                    else:
                        # 扔掉重叠部分，接在上一个 chunk 后面
                        # 这样帧数就是连续的: [0..179] + [180..359] ...
                        start_save = overlap_len
                    
                    # 提取有效部分
                    valid_depth = current_depth[start_save:]
                    valid_poses_c2w = aligned_poses_c2w[start_save:]
                    
                    # 转回 W2C (Extrinsics) 并存入列表
                    for p in valid_poses_c2w:
                        final_w2c_extrinsics.append(np.linalg.inv(p))
                    
                    final_depth.extend(valid_depth)

                # 4. 最终校验与保存
                # 理论上拼接后应该等于 total_len
                # 但由于最后一个 chunk 可能重叠 > 10，我们需要修剪
                # 我们的 Dataset 逻辑中，最后一个 chunk 的 end_idx 强制设为 video_len
                # 倒数第二个 chunk 也是完整的
                # 假设 Video=200, Chunk=180, Overlap=10
                # C0: 0-180. Save 0-180.
                # C1: 170-200. Start=170. Overlap=10. Save 10~30 (即 180-200).
                # Total = 180 + 20 = 200. 完美。
                
                assert len(final_depth) == total_len
                assert len(final_w2c_extrinsics) == total_len
                
                np.savez_compressed(
                    task["npz_path"], 
                    depth=final_depth, 
                    extrinsics=final_w2c_extrinsics
                )
