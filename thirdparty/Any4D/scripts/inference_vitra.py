...
class Any4DDepthEstimator:
    ...
    def prepare_vitra_dataset(self):
        ...
        # 1. 读取大 JSON
        json_path_train = "/share_data/guantianrui/datasets/VITRA-1M/zarr_episodes_statistics.json"
        json_path_test = "/share_data/guantianrui/datasets/VITRA-1M/zarr_episodes_statistics_testset.json"
        print(f"Loading VITRA metadata from train set: {json_path_train} ...")
        print(f"Loading VITRA metadata from test set: {json_path_test} ...")
        
        if not os.path.exists(json_path_train):
            raise FileNotFoundError(f"Metadata file not found: {json_path_train}")

        if not os.path.exists(json_path_test):
            raise FileNotFoundError(f"Metadata file not found: {json_path_test}")
        
        info_train = json.load(open(json_path_train))
        info_test = json.load(open(json_path_test))
        all_video_stats = info_train['video_statistics'] + info_test['video_statistics']
